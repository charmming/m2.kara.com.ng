...
namespace Magezon\Core\Model\Attribute;
...
class ScopeOverriddenValue
{
    ...
    public function containsValue($entityType, $entity, $attributeCode, $storeId)
    {
        ...
        if ($this->attributesValues === null) {
            $this->initAttributeValues($entityType, (int)$storeId);
        }

        return isset($this->attributesValues[$storeId][$entity->getId()])
            && array_key_exists($attributeCode, $this->attributesValues[$storeId][$entity->getId()]);
    }
    ...
    public function getAttributeValues($entityType, $storeId) {
        if ($this->attributesValues === null) {
            $this->initAttributeValues($entityType, (int)$storeId);
        }
     ...
     * @return void
     */
    private function initAttributeValues($entityType, $storeId)
    {
        ...
        if ($metadata->getEavEntityType()) {
            ...
            foreach ($attributeTables as $attributeTable => $attributeCodes) {
                $select = $metadata->getEntityConnection()->select()
                    ->from(['t' => $attributeTable], ['value' => 't.value', 'store_id' => 't.store_id', 'entity_id' => 't.entity_id'])
                    ->join(
            ...
            foreach ($attributes as $attribute) {
                $this->attributesValues[$attribute['store_id']][$attribute['entity_id']][$attribute['attribute_code']] = $attribute['value'];
            }
        }
    }
